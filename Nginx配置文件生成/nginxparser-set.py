...
class NginxConfParse(object):
                ...
                for line in fp:
                    ...
                    if count_config.get("{") > 0:
                        if line.startswith(subkey) and "{" not in line:
                            ...
                            tmp_dict[keyword_name].add(keyline)
    ...
    def _addict(self, inidict, tmdict):
        # 把tmdict中的值增加到inidict数据中
        if len(tmdict) == 0: pass
        for i in tmdict:
            if inidict.get(i):
                inidict[i].add(tmdict[i])
            else:
                inidict[i] = tmdict[i]
        return inidict
...
if __name__ == "__main__":
    ...
    #requests.post('http://192.168.15.106:9999/api/web', json=data)
